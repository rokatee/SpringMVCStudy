...
VALUES (MEMBERRECORDSEQ.NEXTVAL, 10, 11, 12 , 'batwoman');
--> 한 줄 구성
...
SELECT L.ID AS ID
     ...
     ,  NVL(R.ENG,0) AS ENG
     ,  NVL(R.MAT,0) AS MAT
     ,  NVL((R.KOR + R.ENG + R.MAT),0) AS TOT
     ,  NVL(ROUND((R.KOR + R.ENG + R.MAT)/3,2),0) AS AVG
     , RANK() OVER(ORDER BY (R.KOR + R.ENG + R.MAT) DESC)AS RANK
     , L.GRADE
FROM MEMBERLIST L LEFT JOIN MEMBERRECORD R
  ON L.ID = R.ID
ORDER BY NAME;
...
-->
SELECT ID, NAME, TEL, EMAIL, KOR, ENG, MAT, TOT, AVG, RANK FROM MEMBERRECORD_VIEW ORDER BY rank
;